...
if (isset($_POST['modif'])) {
  ...
  if ($nom_categorie != "") {
   ...
   $req = $bdd->prepare('UPDATE categories SET c_nom = :nom_categorie, c_description = :description, c_ref = :ref WHERE id_categorie = :id_categorie');
...
if(isset($_GET['desactiver'])){
  ...
  setFlash('La categorie ' . $nom_categorie .' a bien été désactiver. Cliquez sur <i class="fa fa-check"></i> pour le réactiver.', 'success');
  header('Location:liste_categorie.php');
  die();
}
...
if(isset($_GET['active'])){
  ...
  setFlash('La categorie ' . $nom_categorie .' a bien été activé.  Cliquez sur <i class="fa fa-ban"></i> pour le désactiver.', 'success');
  header('Location:liste_categorie.php');
  die();
}
              ...
              <table  class="table table-bordered table-striped example1">
                ...
                <tbody>
                  ...
                  while ($donnees = $req->fetch()) {
                              ...
                              <div class="box-body">
                                ...
                                <div class="form-group">
                                  ...
                                  <div class="input-group margin">
                                    ...
                                    <input type="text" class="form-control" id="NUM_DECHARGE" name="ref" value="<?php echo $donnees['c_ref']; ?>">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label>Description</label>
                                  <textarea class="form-control" rows="3" placeholder="Entrer ..." name="commentaire"><?php echo $donnees['c_description']; ?></textarea>
                                </div>
                    ...
                    <!--   FIN MODAL   -------------------------------------                    -->
                    <tr>
                      <td><a href="fournisseur.php?p=<?php echo $donnees['id_categorie']; ?>"><?php echo html_entity_decode($donnees['c_nom']); ?></a></td>
                      <td>
                        <?php
                        if ($donnees['c_ref'] == "") {
                          echo "Aucune";
                      ...
                      <td>
                        <a href="categorie.php?p=<?php echo $donnees['id_categorie']; ?>" type="button" class="btn btn-info btn-flat data-placement="top" data-toggle="tooltip" data-original-title="Voir" "><i class="fa fa-search"></i></a>
                        <button type="button" class="btn btn-warning data-placement="top" data-toggle="tooltip" data-original-title="Modifier"" data-toggle="modal" data-target="#<?php echo $donnees['id_categorie']; ?>"><i class="fa fa-edit"></i></button>

                        <?php if ($donnees['c_active'] == 1) {
                          ?>
                          <a href="?desactiver=<?php echo $donnees['id_categorie']; ?>&<?php echo csrf(); ?>" onclick="return confirm('Valider pour désactiver');" type="button" class="btn btn-danger btn-flat data-placement="top" data-toggle="tooltip" href="#" data-original-title="Désactiver le fournisseur" "><i class="fa fa-ban"></i></a>
                          <?php
                        }else{
                          ?>
                          <a href="?active=<?php echo $donnees['id_categorie']; ?>&<?php echo csrf(); ?>" onclick="return confirm('Valider pour activer');" type="button" class="btn btn-success btn-flat data-placement="top" data-toggle="tooltip" href="#" data-original-title="Activer le fournisseur" "><i class="fa fa-check"></i></a>
                          <?php
                        }
